...
# growcar-cover v1.1.14: 封面竞态保护
# 最近一次"含封面"的完整 metadata（由 pushMetadata 缓存）与其版本号。
# 竞态窗口内的无封面推送会被撤销并改用此缓存重建。
...
.method public static bumpCoverRev()V
    .registers 3

    sget v0, Lcom/luna/music/car/CarLyricsBridge;->sCoverRev:I

    const/4 v1, 0x1

    add-int/2addr v0, v1

    sput v0, Lcom/luna/music/car/CarLyricsBridge;->sCoverRev:I

    return-void
.end method

# growcar-cover v1.1.14: 若车载 Session 当前 metadata 丢了 ALBUM_ART（被竞态窗口
# 的无封面推送覆盖），立即用 sCoverMeta 重推一次。只走 setter，无分配无 IO。
# 供后续埋点/心跳调用；当前主修逻辑在 pushMetadata 的撤销检查。
.method public static ensureCoverFlushed()V
    .registers 8
    ...
    :ecf_try_end
    .catchall {:ecf_try_start .. :ecf_try_end} :ecf_catch
...
# growcar-cover v1.1.14: 生成与 sCoverRev 绑定的去重 key（替换固定 "carcover#"），
# 使 sLastPushKey 不会吞掉不同封面版本对同一首歌的重发。
.method private static revAtomicId()Ljava/lang/String;
    .registers 3
    ...
    const-string v1, "carcover#"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    sget v1, Lcom/luna/music/car/CarLyricsBridge;->sCoverRev:I
...
.method public static applyCompat(Landroid/support/v4/media/MediaMetadataCompat;)Landroid/support/v4/media/MediaMetadataCompat;
    ...
    # growcar-cover v1.1.32: 后台自动切歌走 compat(MediaMetadataCompat) 路径，
    # 完全不经过 applyRaw —— 自加载封面必须在这里也挂一个触发点。
    # 按 ALBUM_ART_URI(+标题) 去重，换歌才起一次线程。
    ...
    # 重新取一次 uri 存给 CoverLoadTask 兜底用（v2 已被 key 占用）
    ...
    const-string v2, "COVER kick(compat)"

    invoke-static {v2}, Lcom/luna/music/car/CarLyricsBridge;->logFile(Ljava/lang/String;)V

    new-instance v2, Lcom/luna/music/car/CoverLoadTask;

    const/4 v3, 0x0

    invoke-direct {v2, v3}, Lcom/luna/music/car/CoverLoadTask;-><init>(Ljava/lang/String;)V
...
.method public static ensurePushed()V
    ...
    .line 285
    :cond_3c
    # growcar-cover v1.1.14c: 记录读取基底前的封面版本号（v3 空闲）
    ...
    .line 289
    :try_start_45
    # growcar-cover v1.1.14c: 发布前复查 rev —— 若 apply() 期间有含封面发布落地，
    # v1 是"无封面旧快照+歌词"，直接发布会给车机纯色窗口。改用 sCoverMeta
    #（最后一次含封面版本，apply() 已在其内附加歌词键）。
    sget v2, Lcom/luna/music/car/CarLyricsBridge;->sCoverRev:I

    if-eq v2, v3, :ep_publish_live
...
.method public static setTrackId(J)V
    ...
    :set_track_done
    # growcar-cover v1.1.14: 换歌 → 封面缓存作废（旧封面元数据不得跨歌复用）
    const/4 v0, 0x0
...
# growcar-lrc: MediaSessionCompat metadata 统一出口的「纯增量」装饰器。
#
# v1.1.7 教训 —— 这个方法被 s.P() 在每一次 metadata 推送（含通知栏刷新）时调用，
# 属于绝对热路径，必须遵守三条铁律：
#   1. 没有整段歌词时**原样返回**，绝不写 LYRICS_WHOLE=""/LYRICS_STATUS=2。
#      负状态代表「本曲无歌词」，车机收到后会永久退回单行模式。
#      清空语义只允许在真正切歌的 apply()/applyCompat() 里做。
#   2. 不做任何 side effect（不写 sBaseMeta、不调 ensureSongReset/keepLrcSync/
#      startTickerOnly、不写日志）。通知路径的 metadata 会让 songKey 反复抖动。
#   3. 无歌词窗口零开销 —— 不能进 MediaMetadata$Builder 拷贝（带封面 Bitmap）
#      也不能碰 SharedPreferences/文件 IO，否则首曲会丢封面并出现破音。
#
# 寄存器: v0=sLrc(String) v1=int scratch v2=String scratch v3=Builder/Metadata
#         v4=key(String) v5-v6=long  p0=v7(meta) p1=v8(session)
.method public static applyRaw(Landroid/media/MediaMetadata;Landroid/media/session/MediaSession;)Landroid/media/MediaMetadata;
    ...
    :try_start_raw
    # 已带相同整段歌词（applyCompat 装饰过的路径）-> 不重复拷贝，减少 metadata 抖动
    ...
    invoke-virtual {v3, v4, v0}, Landroid/media/MediaMetadata$Builder;->putString(Ljava/lang/String;Ljava/lang/String;)Landroid/media/MediaMetadata$Builder;
    ...
    if-eqz v5, :raw_no_cache

    sput-object v3, Lcom/luna/music/car/CarLyricsBridge;->sCoverMeta:Landroid/media/MediaMetadata;
...
.method public static publishCoverBitmap(Landroid/graphics/Bitmap;)V
    .registers 6
    ...
    invoke-virtual {v1, v2, p0}, Landroid/media/MediaMetadata$Builder;->putBitmap(Ljava/lang/String;Landroid/graphics/Bitmap;)Landroid/media/MediaMetadata$Builder;
    ...
    const-string v3, "ucar.media.metadata.LYRICS_WHOLE"
    ...
    :pc_done
    :try_end_p
    .catchall {:try_start_p .. :try_end_p} :pc_catch
...
.method public static fetchCoverFromUri(Ljava/lang/String;)Landroid/graphics/Bitmap;
    .registers 4

    # v1.1.29: 在线未缓存歌 MediaStore 无条目，从 metadata 的 ALBUM_ART_URI 直拉。
    # http(s) 走 HttpURLConnection（8s 超时 + UA），其余协议 openStream。全程吞异常。
    :try_start_f
    const/4 v0, 0x0
    ...
    :fc_done
    :try_end_f
    .catchall {:try_start_f .. :try_end_f} :fc_catch
...
.method public static loadCover(Landroid/content/Context;Ljava/lang/String;)Landroid/graphics/Bitmap;
    ...
    invoke-static {v5, v6, v6}, Landroid/graphics/BitmapFactory;->decodeStream(Ljava/io/InputStream;Landroid/graphics/Rect;Landroid/graphics/BitmapFactory$Options;)Landroid/graphics/Bitmap;
...
# growcar-lrc: 每 10s 检查车载 Session 的整段歌词是否被别的 setMetadata 冲掉，冲掉则补推。
# 注意：只用 controller 的**实时** metadata 作为基底，绝不用 sBaseMeta 兜底 ——
# v1.1.7 曾用 sBaseMeta 兜底，而当时 applyRaw 会把通知路径的精简 metadata 写进 sBaseMeta，
# 结果补推时把「无封面无标题」的旧快照推给车机，导致首曲卡片抓不到封面。
...
.method private static shouldResendLrc()Z
    ...
    # v1.1.7 之前这里的判断是反的（elapsed < 25s 才发），导致 ticker 路径
    # 永远不发 lrc_change —— 车机/原子若在播放开始后才连上，就错过唯一一次
    # lrc_change，首曲永远抓不到歌词，直到切歌走 injectExtras 才恢复。
    # 正确语义：距上次发送 >= 25s 才重发。
...
.method private static pushMetadata(Landroid/media/session/MediaSession;)V
    ...
    invoke-virtual {p0, v5}, Landroid/media/session/MediaSession;->setMetadata(Landroid/media/MediaMetadata;)V

    invoke-static {}, Lcom/luna/music/car/CarLyricsBridge;->revAtomicId()Ljava/lang/String;

    move-result-object v5

    invoke-static {p0, v5}, Lcom/luna/music/car/CarLyricsBridge;->pushExtrasTo(Landroid/media/session/MediaSession;Ljava/lang/String;)V

    const-string v5, "pushMetadata: race window detected, lyrics-only revoke sent"
...
.method public static setLyricsFrom(Ljava/lang/Iterable;)V
    ...
    # growcar-cover v1.1.14: 歌词刚被替换 → 作废旧封面缓存（避免旧封面+旧词
    # 锚点被后续 pushMetadata 复用）。不在锁内 flush —— flush 只锦上添花，
    # 真正回填由阶段② s.P() 路径或下一轮 ticker repushMeta 完成。
    const/4 v0, 0x0
